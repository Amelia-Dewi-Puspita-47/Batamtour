...
// Definisi array kategori dengan ikon masing-masing
$kategori = [
...
// Definisi array blog dengan informasi masing-masing blog
$blog = [ 
  [
  "id" => 0,
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    // Mencari data blog berdasarkan ID
    ...
    <div class="container">
        <div class="container1">
            <div class="text-overlayBlog">
                <h1>Detail Blog</h1> <!-- Judul halaman detail blog -->
            </div>
            <img src="assets/images/wisata/Mask.png" class="logo-image" style="height: auto; width:100%;"> <!-- Gambar header -->
            <br>
        </div>
        ...
        <div class="containerblog">
            <div class="blog-left">
              <div class="container1">
                <div class="namaBlog">
                  <h1><?= htmlspecialchars($data['judul']) ?></h1> <!-- Judul blog -->
                  <p><?= $data['tanggal'] ?></p> <!-- Tanggal blog -->
                </div>
                <div class="pic-wisata">
                  <img src="<?= $data['gambar'] ?>" alt="Gambar Blog"> <!-- Gambar blog -->
                </div>
              </div>
              <div class="content-detailblog">
              <p><strong><?= $data['artikel'] ?></strong> - <?= $data['deskripsi'] ?></p> <!-- Artikel dan deskripsi blog -->
              </div>
              <div class="content-author">
              <p><strong><?= $data['penulis'] ?></strong></p> <!-- Penulis blog -->
              <p>Editor: <strong><?= $data['editor'] ?></strong></p> <!-- Editor blog -->
              ...
              <?php if (is_array($data['tagline'])): // Memeriksa apakah 'tagline' adalah array ?>
                    <?php foreach ($data['tagline'] as $tagline): ?>
                        <?php if (isset($kategori[$tagline])): ?>
                            <div class="tag">
                                <img src="<?= htmlspecialchars($kategori[$tagline]) ?>" class="logo-image" style="height: auto; width:13%;"> <!-- Ikon kategori -->
                                <p><?= htmlspecialchars($tagline) ?></p> <!-- Nama kategori -->
                            </div>
                        <?php endif; ?>
                    <?php endforeach; ?>
                <?php endif; ?>
            ...
            <div class="blog-right">
              <h3>Blog terkait</h3> <!-- Judul sidebar blog terkait -->
                <div class="card-berita-container">
                    <?php foreach ($blog as $data): ?> <!-- Loop untuk menampilkan blog terkait -->
                        <a href="<?= htmlspecialchars($data['url']) ?>" class="card-berita">
                            <img src="<?= htmlspecialchars($data['gambar']) ?>" alt="Gambar Blog"> <!-- Gambar blog terkait -->
                            <h2><?= htmlspecialchars($data['judul']) ?></h2> <!-- Judul blog terkait -->
                        </a>
                    <?php endforeach; ?>
                </div>
            </div>
