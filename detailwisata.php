...
<head>
    ...
    <link rel="stylesheet" href="styles.css"> <!-- Menyertakan file CSS untuk gaya halaman -->
</head>
<body>
<?php

// Array untuk ikon kategori
// Array $kategori berisi nama kategori dan path gambar icon yang terkait
$kategori = [
...
// Array untuk data wisata
// Array $wisata berisi detail informasi tentang berbagai tempat wisata
$wisata = [
  [
    "id" => 0,
...
// Mengambil ID dari parameter GET di URL
...
<?php include 'includes/navbar.php'; ?> <!-- Menyertakan file navbar.php -->
    ...
    <div class="container">
        ...
        <div class="container1">
            <div class="namawisata">
                <h1><?= htmlspecialchars($data['nama']) ?></h1> <!-- Menampilkan nama wisata -->
            </div>
        </div>
        <div class="pic-wisata">
          <img src="<?= htmlspecialchars($data['gambarLokasi']) ?>"> <!-- Menampilkan gambar lokasi wisata -->
        </div>
        <div class="lokasi">
            <img src="assets/images/icon/Location.png" alt="Icon Lokasi">
            <p>Lokasi: <?= htmlspecialchars($data['lokasi']) ?></p> <!-- Menampilkan alamat lokasi wisata -->
            <div class="btn-lokasiWisata">
            <button class="detail" onclick="window.location.href = '<?= htmlspecialchars($data['lokasiwisata']) ?>';">Perlihatkan di Peta</button> <!-- Tombol untuk melihat lokasi di peta -->
            </div>
        </div>
        <div class="detail-wisata">
            <p>Kecamatan:</p>
            <div class="tag">
                <p><?= htmlspecialchars($data['kecamatan']) ?></p> <!-- Menampilkan kecamatan -->
            </div>
            <p>Kategori:</p>
            <?php if (is_array($data['tagline'])): // Memeriksa apakah 'tagline' adalah array ?>
                <?php foreach ($data['tagline'] as $tagline): ?>
                    <?php if (isset($kategori[$tagline])): ?>
                        <div class="tag">
                            <img src="<?= htmlspecialchars($kategori[$tagline]) ?>" class="logo-image" style="height: auto; width:13%;"> <!-- Menampilkan icon kategori -->
                            <p><?= htmlspecialchars($tagline) ?></p> <!-- Menampilkan nama kategori -->
                        </div>
                    <?php endif; ?>
                <?php endforeach; ?>
            <?php endif; ?>
            ...
            <div class="informasiwisata">
              <h2>Informasi Wisata</h2>
                <?php if (is_array($data['informasi'])): ?>
                 <?php foreach ($data['informasi'] as $informasi): ?>
                    <p><?= htmlspecialchars($informasi) ?></p> <!-- Menampilkan informasi wisata -->
                 <?php endforeach; ?>
                <?php endif; ?>

              <h2>Event</h2>
              <div class="calender">
                  <!-- Menambahkan kalender di sini -->
                  <iframe src="https://calendar.google.com/calendar/embed?src=your_calendar_id&ctz=Asia/Jakarta" style="border: 0" width="800" height="600" frameborder="0" scrolling="no"></iframe>
              </div>
            
              <h2>Jam Operasional</h2>
              <div class="timeprice">
                  <img src="assets/images/icon/Time.png" class="logo-image" style="height: 3%; width:3%;"> <!-- Icon waktu -->
                  <p><?= htmlspecialchars($data['waktu']) ?></p> <!-- Menampilkan jam operasional -->
              </div>
   
              <h2>Harga Masuk</h2>
              <p>Rp<?= htmlspecialchars($data['harga']) ?>,-</p> <!-- Menampilkan harga masuk -->
              <br>   
              ...
              <?php if (is_array($data['galeri'])): ?>
                  <?php foreach ($data['galeri'] as $galeri): ?>
                    <div class="pht-card">
                      <img src="<?= htmlspecialchars($galeri) ?>"> <!-- Menampilkan gambar galeri -->
                    </div>
                  <?php endforeach; ?>
              <?php endif; ?>
              </div>
...
<?php include 'includes/footer.php'; ?> <!-- Menyertakan file footer.php -->
</body>
</html>
